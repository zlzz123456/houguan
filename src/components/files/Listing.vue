...
<!--  -->
  <div v-else :class="user.viewMode" id="listing"
    class="mosaic"
    @dragenter="dragEnter"
    @dragend="dragEnd">
    <div>
    ...
    <div style="padding-left: 230px" :class="{ active: $store.state.setting.multiple }" id="multiple-selection">
...
export default {
  ...
  methods: {
    ...
    resizeEvent () {
      ...
      let columns = Math.floor(document.querySelector('main').offsetWidth / 300); // {#} -i -sign
    ...
    uploadInput (event) {
      this.checkConflict(event.currentTarget.files, this.req.items, this.fileUrl)
    },
    resetOpacity () {
      let items = document.getElementsByClassName('item')
    ...
    handleFiles (files, base, overwrite = false) {
      ...
      for (let i = 0; i < files.length; i++) {
        let file = files[i]
        promises.push(api.post(base + file.name, file, overwrite, onupload(i)))
      }
