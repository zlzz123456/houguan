...
export function download (format, ...files) {
  ...
  url += `auth=${store.state.setting.jwt}&`

  url += `appId=${store.state.setting.applicationObj.appId}&`

  url += `cId=${store.state.setting.cId}`
...
export async function post (url, content = '', overwrite = false, onupload) {
  ...
  return new Promise((resolve, reject) => {
      ...
      let unzip = store.state.setting.unzip;
      request.open('POST', `${baseURL}/api/resources${url}?unzip=${unzip}&override=${overwrite}&cId=${cId}&appId=${appId}`, true)
      request.setRequestHeader('X-Auth', store.state.setting.jwt)
...
export async function preview () {
  ...
  return new Promise((resolve, reject) => {
    ...
    let cId = store.state.setting.cId;
    let appId = store.state.setting.applicationObj.appId;
    request.open('GET', `${baseURL}/previews/api/rebuild?cId=${cId}&appId=${appId}`, true)
    request.setRequestHeader('X-Auth', store.state.setting.jwt)
